...
contract PairTest is Test {
    ...
    function test_ShouldBeSuccess_burn() public {
        ...
        assertEq(token0.balanceOf(address(this)), TOKEN_A_TOTAL_SUPPLY - 1e18 + token0Amount);
        assertEq(token1.balanceOf(address(this)), TOKEN_B_TOTAL_SUPPLY - 1e18 + token1Amount);
    }

    function test_ShouldBeSuccess_unbalanced_burn() public {
        ...
        uint256 initialPoolBalanceToken0 = token0.balanceOf(address(pair));
        uint256 initialPoolBalanceToken1 = token1.balanceOf(address(pair));
        uint256 token0Amount = (liquidity * initialPoolBalanceToken0) / _totalSupply;
        uint256 token1Amount = (liquidity * initialPoolBalanceToken1) / _totalSupply;
        ...
        assertEq(pair.balanceOf(address(this)), 0);
        assertEq(pair.totalSupply(), pair.MINIMUM_LIQUIDITY());
        assertEq(token0.balanceOf(address(this)), TOKEN_A_TOTAL_SUPPLY - 3e18 + token0Amount);
        assertEq(token1.balanceOf(address(this)), TOKEN_B_TOTAL_SUPPLY - 2e18 + token1Amount);
    }
    ...
    function test_ShouldBeSuccess_swap() public {
        ...
        assertEq(token0.balanceOf(address(this)), TOKEN_A_TOTAL_SUPPLY - 1e18 - swapAmountIn);
        assertEq(token1.balanceOf(address(this)), TOKEN_B_TOTAL_SUPPLY - 2e18 + amountOut);
        (uint112 _reserve0, uint112 _reserve1,) = pair.getReserves();
    ...
    function test_ShouldBeSuccess_reverse_swap() public {
        ...
        assertEq(token0.balanceOf(address(this)), TOKEN_A_TOTAL_SUPPLY - 1e18 + amountOut);
        assertEq(token1.balanceOf(address(this)), TOKEN_B_TOTAL_SUPPLY - 2e18 - swapAmountIn);
        (uint112 _reserve0, uint112 _reserve1,) = pair.getReserves();
    ...
    function test_ShouldBeSuccess_double_swap() public {
        ...
        assertEq(token0.balanceOf(address(this)), TOKEN_A_TOTAL_SUPPLY - 1e18 - swapAmount0In + amountOut);
        assertEq(token1.balanceOf(address(this)), TOKEN_B_TOTAL_SUPPLY - 2e18 - swapAmountIn + amount0Out);
        (uint112 _reserve0, uint112 _reserve1,) = pair.getReserves();
    ...
    function test_Revert_Overflow_sync() public {
        uint256 tokenAmount = type(uint112).max;

        _addLiquidity(tokenAmount, tokenAmount);

        token0.transfer(address(pair), tokenAmount);
        token1.transfer(address(pair), tokenAmount);

        vm.expectRevert(Pair.Overflow.selector);
        pair.sync();
    }
